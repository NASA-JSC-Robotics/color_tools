...
ColorBlobCentroid::ColorBlobCentroid()
    : rclcpp::Node("color_blob_centroid"), m_minBlobSize(10), m_blobSize(-1),
      m_blobSizeThreshold(50), m_blobAspectRatio(-1), m_blobARThreshold(0.03),
      m_imageQos(1), m_color("red"), m_continuousColor(false),
      m_mockHardware(false), m_showImage(false), m_desiredBlob(0) {
  initialize();
}
...
void ColorBlobCentroid::initialize() {
  ...
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Continuous Output set to %s",
              m_continuousColor ? "true" : "false");
  ...
  // depth image topic
  this->declare_parameter("depth_img_topic",
  ...
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Mock Hardware set to %s",
              m_mockHardware ? "true !!! WARNING !!!" : "false");
  ...
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Show Image set to %s",
              m_showImage ? "true" : "false");
  // verbose debug mode that shows underlying color
  this->declare_parameter("debug", false);
  m_debugMode = this->get_parameter("debug").as_bool();
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Debug, set to %s",
              m_debugMode ? "true" : "false");
  ...
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Color image topic: %s",
              ("/" + m_prefix + "/" + m_color_topic).c_str());
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Depth image topic: %s",
              ("/" + m_prefix + "/" + m_depth_topic).c_str());
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Camera info topic: %s",
              ("/" + m_prefix + "/" + m_info_topic).c_str());

  //  --- Image Processing Parameters --
  float dilation_size = 1.0;
  m_morphology = getStructuringElement(
  ...
  m_processing_srv = this->create_service<std_srvs::srv::SetBool>(
      "color_toggle_continuous",
      std::bind(&ColorBlobCentroid::toggle_continuous, this, _1, _2));

  m_imagePub =
      this->create_publisher<sensor_msgs::msg::Image>("colorblob_image", 10);
  m_imageRawPub = this->create_publisher<sensor_msgs::msg::Image>(
      "colorblob_image_raw", 10);
  m_maskPub =
      this->create_publisher<sensor_msgs::msg::Image>("colorblob_mask", 10);

  m_depthImageSub.subscribe(this, "/" + m_prefix + "/" + m_depth_topic,
                            m_imageQos.get_rmw_qos_profile());
  m_colorImageSub.subscribe(this, "/" + m_prefix + "/" + m_color_topic,
                            m_imageQos.get_rmw_qos_profile());
  m_colorInfoSub.subscribe(this, "/" + m_prefix + "/" + m_info_topic,
                           m_imageQos.get_rmw_qos_profile());

  m_timeSyncPtr = std::make_shared<message_filters::TimeSynchronizer<
      sensor_msgs::msg::Image, sensor_msgs::msg::Image,
      sensor_msgs::msg::CameraInfo>>(m_colorImageSub, m_depthImageSub,
                                     m_colorInfoSub, 10);
  m_timeSyncPtr->registerCallback(
      std::bind(&ColorBlobCentroid::imageCallback, this, std::placeholders::_1,
 ...
 *camera
 *****************/
bool ColorBlobCentroid::sendMockHardwareTransform(
    geometry_msgs::msg::PoseStamped &blobPos) {
  if (m_mockHardware) {
    RCLCPP_INFO(
        rclcpp::get_logger("rclcpp"),
        "!!!!! MOCK HARDWARE ENABLED - outputting fake response. !!!!!");
    ...
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"),
                "MSG -> x:0 y:0 z:0.5 --- qx:0 qy:0 qz:0 qw:1 \n\n");
...
/****************
 * Convert cv Image to ROS Image - Given ros header, image encoding, opencv
 *matrix, convert to sensor_msgs::Image
 *****************/
void ColorBlobCentroid::convertCVImageToROS(cv::Mat &input,
                                            const char encoding[],
...
/****************
 * Output Contour- given a contours calculations, prepare it for output, and
 *publish transform
 *****************/
void ColorBlobCentroid::outputContour(geometry_msgs::msg::PoseStamped &blobPos,
                                      double worldX, double worldY,
                                      double depth, double angle) {
  // rotate frame according to long axis -- 90 degrees offset to make  the
  // "vertical long axis" the 0-degree rotation. the gripper opens horizontally,
  // should not need to rotate when object is vertically oriented as that is the
  // correct orientation for grasp.
 ...
 *****************/
void ColorBlobCentroid::processContour(geometry_msgs::msg::PoseStamped &blobPos,
                                       cv::Point2f momentPt,
                                       cv::RotatedRect rotRect) {
  // standardize the height and width regardless of orientation of strip or
  // camera. height is the "longer portion of blob"
  ...
  if (rotRect.size.height > rotRect.size.width)
    angle -= 90;
  if (angle < 0)
    angle += 180; // prevents 180 degrees inversion of gripper for rotation
  ...
  putText(m_colorImage, std::to_string(m_blobNum),
  ...
  double worldX =
      (momentPt.x - m_imageInfo.k.at(2)) *
      (depth /
       m_imageInfo.k.at(
           0)); // (x' - cx) * (depth/focal length x) --- where x' is image x in
                // pixels and cx is center of image x from camera image
  double worldY =
      (momentPt.y - m_imageInfo.k.at(5)) *
      (depth /
       m_imageInfo.k.at(
           4)); // (y' - cy) * (depth/focal length y) --- where y' is image y in
                // pixels and cy is center of image y from camera image
  if (m_debugMode) {
    std::string boxAR = "AR: " + std::to_string((width / height)) +
                        " angle: " + std::to_string(angle);
    std::string boxSize = "W: " + std::to_string(int(width)) +
                          " H:" + std::to_string(int(height));
    // putText(m_colorImage, boxAR, cv::Point2f(momentPt.x - 50, momentPt.y -
    // 10),cv::FONT_HERSHEY_SIMPLEX, 0.5, color, 2);
    putText(m_colorImage, boxSize,
            cv::Point2f(momentPt.x - 50, momentPt.y + 20),
            cv::FONT_HERSHEY_SIMPLEX, 0.5, color, 2);

    std::string worldPos = "world X:" + std::to_string(worldX) +
                           " world Y:" + std::to_string(worldY) +
                           " world Z:" + std::to_string(depth);
    // putText(m_colorImage, worldPos, cv::Point2f(momentPt.x - 50, momentPt.y +
    // 40),cv::FONT_HERSHEY_SIMPLEX, 0.5, color, 2);
  }

  if (doOutput && depth == 0.0) // if the depth camera is too close to the
                                // object, dont publish transforms
  {
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"),
                 "ERROR - Depth Camera too close/far from object.");
    return;
  }
...
bool ColorBlobCentroid::checkValidContour(cv::RotatedRect rotRect) {
  ...
  //  if aspect ratio is set to something specific for testing, only process
  //  those nodes (Aspect/Size == -1) to process all contours that appear
  bool processContour = false;
  if ((m_blobAspectRatio == -1 && m_blobSize == -1) ||
      (m_blobAspectRatio == 0 && m_blobSize == 0)) {
  ...
  if (m_blobAspectRatio != -1 &&
      (width / height) < (m_blobAspectRatio + m_blobARThreshold / 2) &&
      (width / height) > (m_blobAspectRatio - m_blobARThreshold / 2)) {
    // if size exists and meets min size requirement (close enough that it is
    // not considered random noise)
    if (m_blobSize != -1 && height > m_minBlobSize && width > m_minBlobSize &&
        height > (m_blobSize - m_blobSizeThreshold) &&
        height < (m_blobSize + m_blobSizeThreshold)) {
      processContour = true;
    }
  }
...
void ColorBlobCentroid::color_blob_find(
    const std::shared_ptr<dex_ivr_interfaces::srv::BlobCentroid::Request>
        request,
  ...
  if (m_colorImage.empty()) {
    RCLCPP_ERROR(
        rclcpp::get_logger("rclcpp"),
        "ERROR - No image. Check that image topics exist and data is flowing.");
    return;
  }

  // if not mock_hardware actually process the visual node
  ...
  convertCVImageToROS(m_colorImage, sensor_msgs::image_encodings::BGR8,
                      blobImg);
  convertCVImageToROS(m_colorImageRaw, sensor_msgs::image_encodings::BGR8,
                      rawImg);
  convertCVImageToROS(m_mask, sensor_msgs::image_encodings::MONO8, maskImg);
  convertCVImageToROS(m_depthImage, sensor_msgs::image_encodings::TYPE_32FC1,
                      depthImg);
  ...
  if (blobPos.header.frame_id != "") {
    std::string output =
        "Object found at " + std::to_string(blobPos.pose.position.x) + ", " +
        std::to_string(blobPos.pose.position.y) + ", " +
        std::to_string(blobPos.pose.position.z) + ", " +
        " angled: " + std::to_string(blobPos.pose.orientation.x) + ", " +
        std::to_string(blobPos.pose.orientation.y) + ", " +
        std::to_string(blobPos.pose.orientation.z) + ", " +
        std::to_string(blobPos.pose.orientation.w) + "\n\n";
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), output.c_str());
  } else
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"),
                 "FAILED to find object in image frame");
}

/****************
 * Color Blob Set Blob Dimensions Service - advanced service call to get color,
 * has many settable parameters
 *****************/
void ColorBlobCentroid::color_set_blob_dimensions(
    const std::shared_ptr<dex_ivr_interfaces::srv::BlobDimensions::Request>
        request,
    std::shared_ptr<dex_ivr_interfaces::srv::BlobDimensions::Response>
        response) {
  std::string req =
      "Incoming Request -\n Aspect Ratio: " +
      std::to_string(request->aspect_ratio) +
  ...
  if (request->color != "") // change color if given new one
    m_color = request->color;
  if (request->prefix !=
      "") // if new prefix given, change image topic subscribers
  {
...
/****************
 * Toggle Continuous Service - toggle for setting whether you want continuous
 *image processing
 *****************/
void ColorBlobCentroid::toggle_continuous(
    const std::shared_ptr<std_srvs::srv::SetBool::Request> request,
    std::shared_ptr<std_srvs::srv::SetBool::Response> response) {
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Continuous output set to %d",
              request->data);
...
void ColorBlobCentroid::imageCallback(
  ...
  m_colorImage = cv::Mat(cv_bridge::toCvCopy(colorImMsgA, "bgr8")
                             ->image); // this is the opencv encoding
  m_colorImageRaw = cv::Mat(cv_bridge::toCvCopy(colorImMsgA, "bgr8")
                                ->image); // this is the opencv encoding
...
void ColorBlobCentroid::processBlobs(geometry_msgs::msg::PoseStamped &blobPos) {
  ...
  // erode first to delete smol noise outside object of interest, dilate to
  // restore object of interest to proper size
  erode(m_mask, eroded, m_morphology);
  dilate(eroded, dilated, m_morphology);

  // dilates to fill in noise inside object of interest and make it solid. erode
  // to restore object to proper size
  ...
  findContours(eroded, contours, hierarchy, cv::RETR_EXTERNAL,
               cv::CHAIN_APPROX_NONE);
  std::sort(contours.begin(), contours.end(),
            ColorBlobCentroid::sortContour); // sort contours
  ...
  for (size_t i = 0; i < contours.size(); i++) {
    ...
    if (ColorBlobCentroid::checkValidContour(rotRect)) {
      if (m_desiredBlob == m_blobNum) {
        drawContours(m_colorImage,
                     std::vector<std::vector<cv::Point>>(1, contours[i]), -1,
                     cv::Scalar(50, 200, 50), 4, cv::LINE_8);
        drawContours(m_mask, contours, i, cv::Scalar(255, 255, 255), cv::FILLED,
                     cv::LINE_8);
      } else {
        drawContours(m_colorImage,
                     std::vector<std::vector<cv::Point>>(1, contours[i]), -1,
      ...
      if (moment.m00 != 0) {
        ColorBlobCentroid::processContour(
            blobPos,
            cv::Point2f(static_cast<float>(moment.m10 / moment.m00),
  ...
  cv::waitKey(
      1); // set to 1 for continuous output, set to 0 for single frame forever
}
...
void ColorBlobCentroid::advertiseServices() {
  m_color_srv = this->create_service<dex_ivr_interfaces::srv::BlobDimensions>(
      ...
      std::bind(&ColorBlobCentroid::color_set_blob_dimensions, this, _1, _2));
  m_color_simple_srv =
      this->create_service<dex_ivr_interfaces::srv::BlobCentroid>(
          "color_blob_find",
          std::bind(&ColorBlobCentroid::color_blob_find, this, _1, _2));
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"),
              "Ready to process images on service request ");
}
