...
void ColorBlobCentroid::initialize()
{
  ...
  m_morphology = getStructuringElement(cv::MORPH_RECT, cv::Size(2 * dilation_size + 1, 2 * dilation_size + 1),
 ...
 *publish transform
 *****************/
void ColorBlobCentroid::outputContour(geometry_msgs::msg::PoseStamped& blobPos, double worldX, double worldY,
 ...
 *****************/
void ColorBlobCentroid::processContour(geometry_msgs::msg::PoseStamped& blobPos, cv::Point2f momentPt,
                                       cv::RotatedRect rotRect)
{
  ...
  putText(m_colorImage, std::to_string(m_blobNum), cv::Point2f(momentPt.x - 10, momentPt.y - 25),
          cv::FONT_HERSHEY_SIMPLEX, 0.5, color, 2);
  m_blobNum++;
  // Depth image stuff
  double depth = m_depthImage.at<float>(momentPt);
  double worldX = (momentPt.x - m_imageInfo.k.at(2)) *
                  (depth / m_imageInfo.k.at(0));  // (x' - cx) * (depth/focal length x) --- where x' is image x in
                                                  // pixels and cx is center of image x from camera image
  double worldY = (momentPt.y - m_imageInfo.k.at(5)) *
                  (depth / m_imageInfo.k.at(4));  // (y' - cy) * (depth/focal length y) --- where y' is image y in
  ...
  if (m_debugMode)
  {
    ...
    putText(m_colorImage, boxSize, cv::Point2f(momentPt.x - 50, momentPt.y + 20), cv::FONT_HERSHEY_SIMPLEX, 0.5, color,
            2);

    std::string worldPos = "world X:" + std::to_string(worldX) + " world Y:" + std::to_string(worldY) +
...
bool ColorBlobCentroid::checkValidContour(cv::RotatedRect rotRect)
{
  ...
  if (m_blobAspectRatio != -1 && (width / height) < (m_blobAspectRatio + m_blobARThreshold / 2) &&
      (width / height) > (m_blobAspectRatio - m_blobARThreshold / 2))
  {
    ...
    if (m_blobSize != -1 && height > m_minBlobSize && width > m_minBlobSize &&
        height > (m_blobSize - m_blobSizeThreshold) && height < (m_blobSize + m_blobSizeThreshold))
    {
      processContour = true;
    }
  }
                                        ...
                                        std::shared_ptr<dex_ivr_interfaces::srv::BlobCentroid::Response> response)
{
  ...
  {
    std::string output =
        "Object found at " + std::to_string(blobPos.pose.position.x) + ", " + std::to_string(blobPos.pose.position.y) +
        ", " + std::to_string(blobPos.pose.position.z) + ", " +
        " angled: " + std::to_string(blobPos.pose.orientation.x) + ", " + std::to_string(blobPos.pose.orientation.y) +
        ", " + std::to_string(blobPos.pose.orientation.z) + ", " + std::to_string(blobPos.pose.orientation.w) + "\n\n";
...
void ColorBlobCentroid::processBlobs(geometry_msgs::msg::PoseStamped& blobPos)
{
  ...
  for (size_t i = 0; i < contours.size(); i++)
  {
    ...
    if (ColorBlobCentroid::checkValidContour(rotRect))
    {
      ...
      {
        drawContours(m_colorImage, std::vector<std::vector<cv::Point>>(1, contours[i]), -1, cv::Scalar(50, 200, 50), 4,
                     cv::LINE_8);
        drawContours(m_mask, contours, i, cv::Scalar(255, 255, 255), cv::FILLED, cv::LINE_8);
      }
      else
      {
        drawContours(m_colorImage, std::vector<std::vector<cv::Point>>(1, contours[i]), -1, cv::Scalar(0, 255, 255), 1,
                     cv::LINE_8);
      }
      ...
      if (moment.m00 != 0)
      {
        ColorBlobCentroid::processContour(blobPos,
